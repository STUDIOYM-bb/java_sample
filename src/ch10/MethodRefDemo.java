...
//		Mathematical m = d -> Math.abs(d);
		Mathematical m = Math::abs; // static 메서드 참조 방식
		System.out.println(m.calculate(-10.0));

//		Pickable p = (s, i) -> s.charAt(i);
		Pickable p = String::charAt; // 인스턴스 메서드 참조 방식
...
//		Computable c = (i1, i2) -> utils.add(i1, i2);
		Computable c = utils::add; // 인스턴스 메서드 참조 방식
...
//		NewObject<String> s1 = x -> new String(x);
		NewObject<String> s1 = String::new; // 생성자 참조 방식
	}
