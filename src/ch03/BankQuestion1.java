...
public class BankQuestion1 {
	...
	public static void main(String[] args) {
		Scanner sel = new Scanner(System.in); // 입력 전체 설정
		...
		String selMenu = ""; // 메뉴선택 변수명 설정 및 값 초기화 (루프 내에서는 초기화 X)
		...
		while (true) {
			System.out.println("""
					...
					------------------------------------
					선택 > """); // 선택메뉴 목록 출력
			selMenu = sel.nextLine(); // 최초 선택메뉴 입력

			if (selMenu.equals("4")) { // 종료 코드
				System.out.println("프로그램 종료");
				break; // 시스템 종료
			} else if (selMenu.equals("1")) { // 입금 코드
				System.out.println("입금액 > ");
				in = Integer.parseInt(sel.nextLine()); // 입력 된 입금액 in 변수에 추가
				bal = bal + in; // 잔고 변수에 입금액 합산
				System.out.printf("현재 잔고 %d 원 \n", bal); // 잔고 확인

			} else if (selMenu.equals("2")) { // 출금 코드
				System.out.println("출금액 > ");
				out = Integer.parseInt(sel.nextLine());// 입력 된 출금액 out 변수에 추가
			...
			} else if (selMenu.equals("3")) { // 잔액 확인 코드
				System.out.printf("현재 잔고 %d 원 \n", bal); // 잔고 확인
